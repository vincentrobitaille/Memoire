...
```{r}
fit1.3_acc <- fit1.3_gibbs |> 
  filter(accept == 1)

fit1.3_acc$beta0 |> hist()
fit1.3_acc$beta1 |> hist()
fit1.3_acc$sigma2 |> hist()
fit1.3_acc$lambda |> hist()
fit1.3_acc$y3 |> hist()
fit1.3_acc$y9 |> hist()
fit1.3_acc$y12 |> hist()

fit1.3_acc$beta0 |> ts() |> forecast::autoplot()
fit1.3_acc$beta1 |> ts() |> forecast::autoplot()
fit1.3_acc$sigma2 |> ts() |> forecast::autoplot()
fit1.3_acc$lambda |> ts() |> forecast::autoplot()

fit1.3_acc$beta0 |> ts() |> acf(lag.max = 100)
fit1.3_acc$beta1 |> ts() |> acf(lag.max = 100)
fit1.3_acc$sigma2 |> ts() |> acf(lag.max = 100)
fit1.3_acc$lambda |> ts() |> acf(lag.max = 100)
```
